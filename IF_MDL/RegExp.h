...
public:
	...
	{
		int operator () (target_iterator lb, const target_iterator& le, target_iterator rb, const target_iterator& re) const
		{
			if (le - lb > re - rb)
				return 1;
			else if (le - lb < re - rb)
			...
			{
				int ret = 0;
				for (; ret == 0 && lb < le; ++lb, ++rb)
					ret = target_compare()(lb, rb);
				return ret;
			}
		}
	};
	...
	{
		static bool _target_equal(target_iterator lb, const target_iterator& le, target_iterator rb)
		{
			for (; lb != le; ++lb, ++rb)
				if (!target_equal()(lb, rb))
					return false;
			return true;
		}
		bool operator () (const target_iterator& lb, const target_iterator& le, const target_iterator& r) const
		{
			return _target_equal(lb, le, r);
		}
		bool operator () (const target_iterator& lb, const target_iterator& le, const target_iterator& rb, const target_iterator& re) const
		{
			return le - lb == re - rb && _target_equal(lb, le, rb);
		}
	};
	struct target_find
	{
		bool operator () (target_iterator lb, const target_iterator& le, const target_iterator& r) const
		{
			bool ret = false;
			for (; !ret && lb < le; ++lb)
				ret = target_equal()(lb, r);
			return ret;
		}
	};
	...
	public:
		...
		void init_maplist(const target_range& v, map_container& list) const
		{
			target_iterator s = v.first;
			target_iterator c = v.second - 1;
			size_t l = 0;
			for (target_iterator p = s + 1; p < ((target_range)v).second; ++p)
			{
				if (target_equal()(p, c))
				{
					target_range item(target_range(s, p));
					if (std::binary_search(list.begin(), list.end(), map_item(item, l), map_compare()))
						std::lower_bound(list.begin(), list.end(), map_item(item, l), map_compare())->second = l;
					else
						list.insert(std::lower_bound(list.begin(), list.end(), map_item(item, l), map_compare()), map_item(item, l));
					++p;
					++l;
					s = p;
				}
		...
		{
			iregexp_item_tree::const_iterator it = src.root();
			if (std::binary_search((*it)->fmap.begin(), (*it)->fmap.end(), map_item(name, val), map_compare()))
				std::lower_bound((*it)->fmap.begin(), (*it)->fmap.end(), map_item(name, val), map_compare())->second = val;
			else
				(*it)->fmap.insert(std::lower_bound((*it)->fmap.begin(), (*it)->fmap.end(), map_item(name, val), map_compare()), map_item(name, val));
		}
		size_t get_map(typename iregexp_item_tree::const_iterator src, const target_range& name) const
		{
			iregexp_item_tree::const_iterator it = src.root();
			if (!std::binary_search((*it)->fmap.begin(), (*it)->fmap.end(), map_item(name, 0), map_compare()))
				return -1;
			map_container::const_iterator i = std::lower_bound((*it)->fmap.begin(), (*it)->fmap.end(), map_item(name, 0), map_compare());
			return i->second;
		}
		...
		void get_map_string(typename iregexp_item_tree::const_iterator src, std::wstring& str) const
		{
			iregexp_item_tree::const_iterator it = src.root();
			wchar_t istr[33];
			map_container::const_iterator i = (*it)->fmap.begin();
			while (i != (*it)->fmap.end())
			{
				str += L"[";
				str.append((wchar_t*)&*((target_range)i->first).first, ((target_range)i->first).second - ((target_range)i->first).first);
				str += L"=";
	...
	: public iregexp_base<target_iterator, target_compare>
{
	...
	protected:
		virtual size_t get_min_match_size_(void) { return target_range_size()(ftarget); }
		virtual size_t get_max_match_size_(void) { return target_range_size()(ftarget); }
		virtual bool can_backtrack(void) { return false; }
		virtual bool _syntax(typename iregexp_item_tree::const_iterator src, const target_iterator vb, target_iterator &vp, const target_iterator ve, bool backtracking)
		{
	...
	public:
		...
		{
			str += L"str_p('";
			for (target_iterator i = ((target_range)ftarget).first; i < ((target_range)ftarget).second; ++i)
			{
				if (*((wchar_t*)&*i) < 0x20)
					break;
	...
	: public iregexp_base<target_iterator, target_compare>
{
	...
	protected:
		...
		virtual bool _syntax(typename iregexp_item_tree::const_iterator src, const target_iterator vb, target_iterator &vp, const target_iterator ve, bool backtracking)
		{
			bool ret = target_range_size()(ftarget) > 0 && vp < ve && (fnot ^ target_find()(ftarget.first, ftarget.second, vp));
			if (ret)
				++vp;
			return ret;
		}
	...
	: public iregexp_base<target_iterator, target_compare>
{
		...
		virtual bool _syntax(typename iregexp_item_tree::const_iterator src, const target_iterator vb, target_iterator &vp, const target_iterator ve, bool backtracking)
		{
			if (!backtracking)
			{
				...
				if (!it.is_end() && (**it).is_matchs())
				{
					...
					if (std::binary_search(flist.begin(), flist.end(), map_item(item, 0), map_compare()))
					{
						map_container::const_iterator s = std::lower_bound(flist.begin(), flist.end(), map_item(item, 0), map_compare());
	...
	public:
		...
		virtual void get_type_string(std::wstring& str) const
		{
			...
			case ctName:
				str += L"'";
				str.append((wchar_t*)&*((target_range)fname).first, target_range_size()((target_range)fname));
				str += L"'";
				break;
			}
		}
...
private:
	...
	public:
		...
		virtual void get_type_string(std::wstring& str) const
		{
			...
			str += istr;
			str += L"=>";
			str.append((wchar_t*)&*((target_range)fvar_name).first, target_range_size()((target_range)fvar_name));
			str += L")";
		}
	};
	...
	public:
		regexp_map_variable_item(size_t index, const target_range& list, const target_range& var_name)
		{
			fcapture_type = ctIndex;
			findex = index;
			fvar_name = var_name;
		...
		regexp_map_variable_item(size_t index, bool prev, const target_range& list, const target_range& var_name)
		{
		...
		regexp_map_variable_item(const target_range& name, const target_range& list, const target_range& var_name)
		{
			fcapture_type = ctName;
			fname = name;
			fvar_name = var_name;
		...
		virtual void get_type_string(std::wstring& str) const
		{
			...
			map_container::const_iterator i = flist.begin();
			while (i != flist.end())
			{
				str += L"[";
				_itow_s(i->second, istr, 10);
				str += istr;
				str += L",";
				str.append((wchar_t*)&*((target_range)i->first).first, target_range_size()((target_range)i->first));
				str += L"]";
				++i;
			}
			str += L"=>";
	...
	: public iregexp_base<target_iterator, target_compare>
{
	...
	protected:
		virtual bool _syntax(typename iregexp_item_tree::const_iterator src, const target_iterator vb, target_iterator &vp, const target_iterator ve, bool backtracking)
		{
			...
			{
				iregexp_item_point pos = (**it).last_matchs();
				target_range item(target_range(vb + pos.first, vb + pos.second));
				if (std::binary_search(flist.begin(), flist.end(), map_item(item, 0), map_compare()))
				{
					map_container::const_iterator s = std::lower_bound(flist.begin(), flist.end(), map_item(item, 0), map_compare());
