...
public:
	...
	virtual void OnIfAction(long param, bool success, const target_iterator vb, target_iterator &vp, const target_iterator ve)
	{
		...
		case rrpsParam:		fparam = success ? fvender.number(std::pair<target_iterator, target_iterator>(vp, ve)) * (fsign_minus ? -1 : 1) : 0; break;
		}
	}
	virtual void OnAction(long param, const target_iterator vb, target_iterator &vp, const target_iterator ve)
	{
		switch (param)
		{
		case rrpsXParam:		fparam = fvender.xnumber(std::pair<target_iterator, target_iterator>(vp, ve)); break;
		}
	}
	virtual void OnFunc(long param, const target_iterator vb, target_iterator &vp, const target_iterator ve)
	{
		...
		case rrpsSetChar:
		{
			std::pair<size_t, size_t> pt = fparser.matchs(fvender.str<target_iterator>("char"));
...
public:
	iregexp_rtf_parser(iregexp_vender& vdr, const iregexp_builder& reg)
		: fvender(vdr)
	{
